...
def load_asset_text(filename: str) -> str:
    """Load a frontend asset from the assets folder."""
...
style_content = load_asset_text("brahmastra_styles.css")
if style_content:
...
brand_template = load_asset_text("brahmastra_branding.html")
if brand_template:
    st.markdown(brand_template.replace("{{brand_img_tag}}", brand_img_tag), unsafe_allow_html=True)
else:
    st.markdown(
        f"""
...
if st.session_state.request_location:
    geo_js = load_asset_text("brahmastra_geolocation.js")
    if geo_js:
        components.html(f"<script>{geo_js}</script>", height=0)
