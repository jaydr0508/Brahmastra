...
BRAND_IMAGE_PATH = os.path.join(BASE_DIR, "assets", "brahmastra_brand.jpg")
...
brand_b64 = get_image_base64(BRAND_IMAGE_PATH)


# ==================== MODULAR SERVICES ARCHITECTURE ====================

# 1. Image Generation Engine Interface & Providers
...
class PollinationsImageGenerator(ImageGenerator):
    """Reliable, high-quality AI diffusion image generation provider."""
    def generate(self, prompt: str) -> dict:
        try:
            encoded = urllib.parse.quote(prompt.strip())
            url = f"https://image.pollinations.ai/prompt/{encoded}?width=768&height=768&nologo=true"
            req = urllib.request.Request(url, headers={"User-Agent": "Brahmastra-AI/1.0"})
            with urllib.request.urlopen(req, timeout=25) as resp:
                data = resp.read()
                b64 = base64.b64encode(data).decode("utf-8")
                return {"success": True, "image_b64": b64, "mime_type": "image/jpeg", "error": None}
        except Exception as e:
            return {"success": False, "image_b64": None, "mime_type": None, "error": str(e)}


class GeminiImageGenerator(ImageGenerator):
    """Gemini API image generation provider when enabled with valid quota."""
    ...
    def generate(self, prompt: str) -> dict:
        try:
            ...
            return {"success": False, "image_b64": None, "mime_type": None, "error": "No image data returned from Gemini"}
        except Exception as e:
            return {"success": False, "image_b64": None, "mime_type": None, "error": str(e)}


class CompositeImageGenerator(ImageGenerator):
    """Tries primary provider first and falls back gracefully to secondary provider."""
    def __init__(self, providers: list[ImageGenerator]):
    ...
    def generate(self, prompt: str) -> dict:
        ...
        return {"success": False, "image_b64": None, "mime_type": None, "error": last_error or "Image generation failed"}


# Instantiate modular image generator
image_generator = CompositeImageGenerator([
    GeminiImageGenerator(client),
...
st.markdown(
    ...
    .chat-generated-image {
        ...
        box-shadow: 0 8px 32px rgba(0, 0, 0, 0.5), 0 0 25px rgba(37, 99, 235, 0.35);
        transition: transform 0.3s ease, box-shadow 0.3s ease;
    ...
    .chat-generated-image:hover {
        ...
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.6), 0 0 35px rgba(59, 130, 246, 0.5);
    }
...
# 1. BRAHMĀSTRA BRANDING HEADER (IMAGE DIRECTLY BEFORE HEADING)
if brand_b64:
    brand_img_tag = f'<img src="data:image/jpeg;base64,{brand_b64}" class="branding-img" alt="Brahmāstra Golden Lotus" />'
else:
    brand_img_tag = ""
...
if not st.session_state.messages:
    welcome_text = (
        ...
        f"✨ You can chat with me, explore research & projects, or switch to **Create Image** mode below to generate celestial concept art!"
    )
...
for idx, msg in enumerate(st.session_state.messages):
    with st.container(border=True):
        if msg.role == "user":
            ...
            st.markdown(f"**🏹 Brahmāstra**\n\n{msg.parts[0].text}")
            # Render attached generated image if present
                    ...
                    st.markdown(
                        ...
                        <div class="chat-image-wrapper">
                            <img src="data:{att['mime']};base64,{att['b64']}" 
                                 ...
                                 alt="{att['prompt']}" />
                            <div class="chat-image-caption">✨ Prompt: "{att['prompt']}"</div>
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )

# ==================== INTERACTION AREA & MODE CONTROLS ====================

# Control bar above input: Mode Toggle (Chat | Create Image) + Location Button
st.markdown("---")
ctrl_col1, ctrl_col2, ctrl_col3 = st.columns([2.5, 3.5, 4])

with ctrl_col1:
    chat_btn_type = "primary" if st.session_state.input_mode == "chat" else "secondary"
    if st.button("💬 Chat", type=chat_btn_type, use_container_width=True, key="btn_mode_chat"):
        st.session_state.input_mode = "chat"
        st.rerun()

with ctrl_col2:
    img_btn_type = "primary" if st.session_state.input_mode == "image" else "secondary"
    if st.button("🎨 Create Image", type=img_btn_type, use_container_width=True, key="btn_mode_image"):
        st.session_state.input_mode = "image"
        st.rerun()

with ctrl_col3:
    if st.session_state.user_location:
        city_display = st.session_state.user_location['display'].split(',')[0]
        if st.button(f"📍 {city_display} ✕", use_container_width=True, key="btn_loc_clear", help="Click to clear detected location"):
            st.session_state.user_location = None
            st.session_state.geo_error = None
            st.rerun()
    else:
        if st.button("📍 Location", use_container_width=True, key="btn_loc_req", help="Detect your location with browser permission"):
...
if st.session_state.input_mode == "image":
    with st.container(border=True):
        st.markdown("#### 🎨 **BRAHMĀSTRA Image Generation Studio**")
        st.caption("Describe the scene, educational concept, or concept art you want BRAHMĀSTRA to synthesize:")

        img_prompt = st.text_area(
            "Image Prompt:",
            placeholder="e.g. A futuristic blue holographic AI interface, cinematic digital art, celestial lighting, 8k resolution...",
            key="img_prompt_input",
            height=90,
            label_visibility="collapsed",
        )

        gen_col1, gen_col2, gen_col3 = st.columns([3, 2, 2])
        with gen_col1:
            start_generate = st.button("✨ Generate Image", type="primary", use_container_width=True)
        with gen_col2:
            if st.session_state.last_generated_image:
                if st.button("🔄 Regenerate", use_container_width=True):
                    img_prompt = st.session_state.last_generated_image["prompt"]
                    start_generate = True
        with gen_col3:
        ...
        if start_generate:
            if not img_prompt.strip():
            ...
            elif check_18_plus_content(img_prompt):
                st.error("I am not allowed to generate imagery for this topic.")
            else:
                # Elegant futuristic Brahmastra-style generation animation placeholder
                ...
                anim_placeholder.markdown(
                    ...
                    <div class="image-generating-anim">
                        ...
                        <div class="generating-desc">Synthesizing visual concepts into celestial imagery</div>
                ...
                # Invoke modular image generator
                result = image_generator.generate(img_prompt)
                ...
                if result.get("success"):
                    ...
                    user_content = types.Content(role="user", parts=[types.Part.from_text(text=f"🎨 Create Image: {img_prompt}")])
                    st.session_state.messages.append(user_content)

                    # 2. Append assistant message to conversation history
                    assistant_msg = f"I have synthesized the image based on your prompt: **\"{img_prompt}\"**"
                    model_content = types.Content(role="model", parts=[types.Part.from_text(text=assistant_msg)])
                    st.session_state.messages.append(model_content)

                    # 3. Associate image attachment with assistant message
                    ...
                    st.error(f"Image generation failed: {result.get('error', 'Unknown error')}. Please try again.")

        # Preview of last generated image with controls
        if st.session_state.last_generated_image:
            st.markdown("---")
            st.markdown("##### 🖼️ **Generated Image Preview**")
            last_img = st.session_state.last_generated_image
            st.markdown(
                f"""
                <div class="chat-image-wrapper">
                    <img src="data:{last_img['mime']};base64,{last_img['b64']}" 
                         class="chat-generated-image" 
                         alt="{last_img['prompt']}" />
                    <div class="chat-image-caption">✨ {last_img['prompt']}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
elif st.session_state.input_mode == "chat":
    prompt = st.chat_input("Ask Brahmāstra anything...")
    if prompt:
        ...
        # 2. Priority 1: 18+ Content Safety Filter Layer
        ...
        # 3. Priority 2: Location Direct Answer Layer
            ...
            if st.session_state.user_location:
                ...
                loc_reply = "Location access hasn't been granted. Would you like to allow location access? You can click the 📍 Location button above to enable it."
        ...
        # 4. Priority 3: SDG Focus Mode Redirection
        ...
        # 5. Priority 4: Gemini Model Response Generation
        else:
            ...
            with st.spinner("Brahmāstra is thinking..."):
                try:
                    ...
                    error_msg = f"⚠️ An error occurred while communicating with Gemini: {str(e)}"
                    st.error(error_msg)